...
*   [Windows 系统部署](#windows-系统部署)
    ...
    *   [基础运行（Python 环境，备用）](#基础运行python-环境备用)

    *   [打包为 EXE 可执行文件（备用）](#打包为-exe-可执行文件备用)

    *   [注册为系统服务（开机自启，备用）](#注册为系统服务开机自启备用)
...
使用 `DNS-TXT-Proxy-Manager.exe` 图形客户端，系统托盘后台运行，无需 Python 环境。
...
**使用方式：**
1. 下载 `DNS-TXT-Proxy-Manager.exe` 直接运行
2. 首次运行会自动拷贝内置默认配置，也可通过工具栏「📁 配置文件」选择已有 `config.conf`
3. 点击「＋ 添加」输入名称、域名、端口等信息添加映射
4. 修改 `config.conf` 后点击「⟳ 刷新」重新加载

**（可选）如果需要手动打包（需 Python + PyInstaller）：**
```bash
